...
def part_ii_interface(filename: str, kss: Dict[str, List[int]]) -> None:
    ...
    prompt = '''What would you like to do?
        ...
        - Most (P)ositive words
        - Most (N)egative words
        - (Q)uit
: '''    
    while True:
        ...
        if command == 'q':
            ...
            mpw = most_positive_words(word_count, occ_count, kss)
            print(word_count, 'most positive words that occur at least', occ_count, 'times:')
            for item in mpw:
            ...
            word_count = int(input('How many words would you like?: '))
            occ_count = int(input('Least number of occurrences?: '))
            mpw = most_negative_words(word_count, occ_count, kss)
            print(word_count, 'most negative words that occur at least', occ_count, 'times:')
            for item in mpw:
                print(item[0], '\t', round(item[1],2), '\t(seen '+ str(item[2]) + ' times)')
        else:
            print('Command not found')
